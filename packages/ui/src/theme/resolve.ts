...
import { blend, generateNeutralScale, generateScale, hexToOklch, hexToRgb, oklchToHex, shift, withAlpha } from "./color"
...
export function resolveThemeVariant(variant: ThemeVariant, isDark: boolean): ResolvedTheme {
  ...
  // ─── Prax-dev: Accent seeds with fallbacks (for aurora/glow/glass tokens) ───
  const seeds = variant.seeds
  const accentSeed = seeds?.accent ?? colors.interactive
  const accentSecondarySeed = seeds?.accentSecondary ?? colors.info
  const accentTertiarySeed = seeds?.accentTertiary ?? (seeds?.warning ?? colors.warning)
