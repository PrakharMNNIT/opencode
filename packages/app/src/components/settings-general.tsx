...
export const SettingsGeneral: Component = () => {
  ...
  sdk.client.config.get().then((res) => {
    const strategy = res.data?.compaction?.thinking_strategy
      ...
      <div class="bg-surface-raised-base px-4 rounded-lg">
        ...
        >
          <Select
            ...
            onSelect={(option) => {
              if (!option) return
              const prev = thinkingStrategy()
              setThinkingStrategy(option.value)
              sdk.client.config
                .update({ config: { compaction: { thinking_strategy: option.value } } })
                .then(() =>
                  showToast({
                    variant: "success",
