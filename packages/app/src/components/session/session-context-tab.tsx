        ...
        <Show when={ctx()}>
          {(c) => {
            ...
            sdk.client.config.get().then((res) => {
              const val = res.data?.compaction?.thinking_strategy
              if (val === "none" || val === "strip" || val === "compact") setStrategy(val)
            }).catch((err) => console.error("Failed to load thinking strategy config", err))
            ...
            const updateStrategy = (value: "none" | "strip" | "compact") => {
              ...
              setStrategy(value)
              sdk.client.config
                .update({ config: { compaction: { thinking_strategy: value } } })
                .then(() => showToast({
                  variant: "success",
                  ...
                  description: value === "strip"
                    ...
                    : value === "compact"
                      ...
                      : "Thinking blocks sent as-is (original behavior)",
                }))
                .catch((err) => {
                  setStrategy(prev)
            ...
            const stripAndCompact = async () => {
              ...
              setStripping(true)
              try {
                await sdk.client.config.update({ config: { compaction: { thinking_strategy: "strip" } } })
                const last = visibleUserMessages().at(-1)
                await sdk.client.session.summarize({
                  sessionID: params.id,
                  directory: sdk.directory,
                ...
                showToast({
                  variant: "success",
                  title: "Strip & compact",
                  description: "Switched to strip strategy. Session compacting...",
                })
              } catch (err) {
                showToast({
                  title: "Strip & compact failed",
